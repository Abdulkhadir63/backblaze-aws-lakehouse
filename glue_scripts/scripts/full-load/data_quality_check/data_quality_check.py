...
def load_yaml_from_s3(s3_uri):
    """
    Load YAML configuration from S3.
    """

    content = load_s3_text(s3_uri)
...
def normalize_datatype(datatype):
    """
    Normalize common datatype naming differences between
    the canonical YAML definition and Spark's schema strings.
    """

    if datatype is None:
        return None

    value = str(datatype).strip().lower()
...
canonical_columns = schema_config.get(
    "columns",
    []
)
...
# =========================================================
# 6. READ SILVER WITH EXPLICIT RELEASE SCOPE
# =========================================================
...
# =========================================================
#
# We calculate the project-specific metrics in ONE
# Spark aggregation.
#
# Date range / continuity metrics are intentionally NOT
# calculated here.
#
# We do not require:
#   MIN(snapshot_date)
...
#
# because releases may legitimately have missing dates
...
distinct_business_keys = (
    aggregate_row["distinct_business_keys"]
)
...
def add_result(
    ...
    """
    Add one standardized DQ result.
    """
...
if total_rows > 0:
    ...
    for row in glue_rule_rows:
        ...
        details = (
            f"rule={row['Rule']}; "
            f"failure_reason={row['FailureReason']}; "
            f"metrics={row['EvaluatedMetrics']}"
        )
...
if total_rows > 0:
    ...
    if (
        ...
        add_result(
            check_name="business_key_uniqueness",
            result="PASS",
            details=(
                f"rows={total_rows}; "
                f"distinct_keys="
...
for column_name, null_count in (
    critical_null_metrics.items()
):
...
unexpected_columns = [
    ...
    for column_name in actual_schema

    if (
        column_name not in expected_schema
        and
        column_name not in operational_columns
    )
]
...
if (
    ...
    add_result(
        ...
        result="PASS",
        details=(
            "All canonical columns exist with "
            "compatible datatypes. "
            "Known operational metadata columns "
            "are permitted."
        )
    )
...
# =========================================================
#
# This remains a SANITY reconciliation.
#
# Silver can be lower than Bronze because Silver:
#   - removes invalid required-field records
#   - deduplicates the business observation key
#
# We reject only:
#
#       Silver > Bronze
...
print(
    f"DQ Results Table: {dq_results_table}"
)
