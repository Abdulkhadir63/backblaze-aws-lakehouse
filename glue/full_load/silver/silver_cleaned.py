...
schema_config = load_yaml_from_s3(schema_s3_uri)

canonical_columns = schema_config["columns"]
...
# ---------------------------------------------------------
# 4. READ ONLY THE REQUESTED RELEASE FROM BRONZE
# ---------------------------------------------------------
df_bronze = (
...
# Fail early if requested scope is empty.
if df_bronze.limit(1).count() == 0:
    raise ValueError(
        f"No Bronze data found for release_id={release_id}"
    )
...
def build_source_expression(field, source_columns):
    ...
    if source not in source_columns:
        ...
        raise ValueError(
            f"Required source column '{source}' is missing "
            f"for canonical field '{target}'"
        )
    ...
    if target in {
        ...
        source_col = F.trim(source_col)

        source_col = F.when(
            F.length(source_col) == 0,
            F.lit(None)
        ).otherwise(source_col)

    # Backblaze `date` is a DATE, not a TIMESTAMP.
    if source == "date":

        return (
            F.to_date(source_col, "yyyy-MM-dd")
            .alias(target)
        )
    ...
    if transformation == "to_timestamp":
        ...
        return (
            F.to_timestamp(source_col, "yyyy-MM-dd")
            .alias(target)
        )
...
source_columns = set(df_bronze.columns)
...
missing_required_fields = [
    ...
    and field.get("source_column") not in source_columns
]
...
if missing_required_fields:
    ...
    if not spark.catalog.tableExists(quarantine_table):
        ...
        (
            df_quarantine.writeTo(quarantine_table)
    ...
    raise ValueError(
        ...
        f"Records were quarantined in {quarantine_table}. "
        f"Missing fields: {missing_required_fields}"
    )
...
#
# ONLY CHANGE:
# Build rejection reasons as a flat array instead of
# incrementally constructing a deeply nested OR expression.
#
# The MERGE section below is intentionally untouched.
# ---------------------------------------------------------
...
for field in canonical_columns:
    ...
    if target in {
        ...
        parsed_value = (
            source_col.cast(field["datatype"])
        )
...
if required_rejection_expressions:
    ...
    # Build one flat array containing only non-null
    # rejection reasons.
    reasons_array = F.array_compact(
        F.array(
            *required_rejection_expressions
        )
    )
...
df_silver_candidate = (
    ...
    .select(
        *canonical_expressions,
        ...
        "source_file",
        "ingested_at",
        invalid_condition.alias("_is_invalid"),
...
if not spark.catalog.tableExists(
    ...
    # Do not modify this unless we explicitly redesign
    # the Silver write strategy.
    ...
    merge_query = f"""
        ...
        ON target.release_id = {formatted_release_id}
           AND target.release_id = source.release_id
           AND target.snapshot_date = source.snapshot_date
           AND target.drive_serial_number = source.drive_serial_number
...
# Release persisted candidate after quarantine
# and Silver processing have consumed it.
